  ...
  case 7:
    ...
    case PID_TOPIC_NAME:
      if (offset < 4) {
        ...
        if (ep_type & APP_EP_PUB) {
          if (offset < sp_len + 4 && pub_topic_name[offset - 4] != data) {
            flags |= (hls_uint<5>)FLAGS_UNMATCH_TOPIC;
          }
        } else {
          if (offset < sp_len + 4 && sub_topic_name[offset - 4] != data) {
            flags |= (hls_uint<5>)FLAGS_UNMATCH_TOPIC;
          }
        }
      }
    ...
    case PID_TYPE_NAME:
      if (offset < 4) {
        ...
        if (ep_type & APP_EP_PUB) {
          if (offset < sp_len + 4 && pub_type_name[offset - 4] != data) {
            flags |= (hls_uint<5>)FLAGS_UNMATCH_TYPE;
          }
        } else {
          if (offset < sp_len + 4 && sub_type_name[offset - 4] != data) {
            flags |= (hls_uint<5>)FLAGS_UNMATCH_TYPE;
          }
        }
      }
